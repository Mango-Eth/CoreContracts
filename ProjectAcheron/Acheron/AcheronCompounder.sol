...
import "./AcheronOracle.sol";
import {IAcheronCompounder} from "./interfaces/IAcheronCompounder.sol";

contract AcheronCompounder is AcheronOracle, IAcheronCompounder {
    ...
    function _aggregate_(uint256 _skirkAmount, uint256 _wethAmount, uint256 amount2Spend, uint8 flag) internal returns(uint256){
        ...
        if(sqrtP_n < LG){
            uint256 wethAmount_ = _daiToWeth(wethPrice, (totalAmount - totalAmount.mulDiv(21, 1000)));
            ...
            // }
            liquidity = getLiquidity(totalAmount, wethPrice, sqrtP_n);  // New one.
        }
        ...
        return (skirkRemaining); // 212201542541304090 + 88638649665945184
    }
    ...
    */  
    ...
    function getSum0(uint256 amount, uint256 wethPrice, uint160 sqrtP_n) internal returns(uint256){
    ...
    function getSum1(uint256 amount, uint256 wethPrice, uint160 sqrtP_n) internal returns(uint256){
    ...
    event Diffs(uint256);
    function getLiquidity(uint256 amount2Spend, uint256 wethPrice, uint160 sqrtP_n) internal returns(uint128){
        uint128 fl;
        uint256 target = amount2Spend - amount2Spend.mulDiv(4, 100);
        uint256 sum0 = getSum0(target, wethPrice, sqrtP_n);
        uint256 sum1 = getSum1(target, wethPrice, sqrtP_n);
        emit Sum0(sum0);
        emit Sum1(sum1);
